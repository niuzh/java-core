	...
	public static void main(String[] args) throws Exception {
		...
		try (InputStream styleIn = Files.newInputStream(path)) {
			...
			try (InputStream docIn = Files.newInputStream(Paths.get("transform", "employee.dat"))) {
				//构建SAX数据源，使用指定的阅读器解析输入流
				SAXSource saxSource=new SAXSource(new EmployeeReader(), new InputSource(docIn));
				t.transform(saxSource, new StreamResult(System.out));
			}
		}
	}
}
