...
public class ScriptTest {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					final String frameClassName = args.length < 2 ? "v2.ch10.buttons1.ButtonFrame" : args[1];
	...
	*/
	private static void addListener(String beanName, String eventName, final String scriptCode, final ScriptEngine engine) throws ReflectiveOperationException, IntrospectionException {
		Object bean = engine.get(beanName);
		EventSetDescriptor descriptor = getEventSetDescriptor(bean, eventName);
		if (descriptor == null)
			return;
		descriptor.getAddListenerMethod().invoke(bean, Proxy.newProxyInstance(null, new Class[] { descriptor.getListenerType() }, new InvocationHandler() {
